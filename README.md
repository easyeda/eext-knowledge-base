...
#### 本地模型支持类型
本地模型支持使用ONNX模型，若镜像站无法拉取模型文件，可本地导入ONNX模型，导入后文件会存入浏览器缓存，建议导入时只选择需要使用的模型文件，避免占用空间。

#### 向量模型说明
重构向量需要耗费大量时间，建议使用推荐向量模型，可直接使用预构建向量，避免全文件向量重构。

#### LLM模型说明
LLM模型推荐使用4B模型才会有良好体验，需要注意的是本地LLM运行会占用大量内存，确保电脑有足够内存用于推理。
