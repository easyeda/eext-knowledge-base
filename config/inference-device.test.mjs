...
const { fitLocalPrompt, localInputTokens } = await load('iframe/src/local-prompt.ts');
const { chatModelName, embeddingModelName, remoteModelDtype, DEFAULT_CHAT_MODEL, DEFAULT_EMBEDDING_MODEL } = await load('iframe/src/remote-models.ts');

it('migrates old defaults and selects available precision on GPU and CPU fallback', () => {
	assert.equal(chatModelName(), DEFAULT_CHAT_MODEL);
	assert.equal(chatModelName('onnx-community/Qwen2.5-0.5B-Instruct'), DEFAULT_CHAT_MODEL);
	assert.equal(embeddingModelName('Xenova/bge-large-zh-v1.5'), DEFAULT_EMBEDDING_MODEL);
	assert.equal(chatModelName('custom/model'), 'custom/model');
	assert.equal(remoteModelDtype(DEFAULT_CHAT_MODEL, 'q8', 'webgpu'), 'fp16');
	assert.equal(remoteModelDtype(DEFAULT_CHAT_MODEL, 'q8', 'wasm'), 'fp32');
	assert.equal(remoteModelDtype(DEFAULT_CHAT_MODEL, undefined, 'wasm'), 'fp32');
	assert.equal(remoteModelDtype(DEFAULT_EMBEDDING_MODEL, 'q8', 'webgpu'), 'q8');
});
...
it('bounds long prompts without changing the current question or the original history', () => {
	assert.equal(localInputTokens(), 131072);
	assert.equal(localInputTokens(8192), 8192);
	assert.equal(localInputTokens(-1), 131072);
	assert.equal(localInputTokens(Number.NaN), 131072);
	const input = [{ role: 'system', content: 'reference'.repeat(100) }, { role: 'user', content: 'old question' }, { role: 'assistant', content: 'old answer' }, { role: 'user', content: 'current question' }];
	const count = messages => messages.reduce((sum, message) => sum + message.content.length + 4, 0);
	const output = fitLocalPrompt(input, count, 80);
	assert(count(output) <= 80);
	assert.equal(output.length, 2);
	assert.deepEqual(output.at(-1), input.at(-1));
	assert.equal(input[0].content.length, 900);
	assert.deepEqual(fitLocalPrompt(output, count, 80), output);
	assert.throws(() => fitLocalPrompt(input, count, 10), /输入上限/);
});

for (const stalledStage of ['init', 'generate']) {
	it(`terminates a stalled ${stalledStage} and allows retry without automatic CPU replay`, async (t) => {
		const all = workers(t, (worker, request) => {
			if (all.length > 1 || request.type !== stalledStage)
				worker.send(request, 'done');
		});
		const client = new InferenceClient({ kind: 'text-generation', device: 'auto', generationTimeoutMs: 20, initializationTimeoutMs: 20 });
		t.after(() => client.dispose());
		await assert.rejects(client.run('generate'), { name: 'InferenceTimeoutError' });
		assert.equal(all.length, 1);
		assert.equal(all[0].terminated, true);
		await client.run('generate');
...
it('reuses a successfully initialized chat model across questions', async (t) => {
	const all = workers(t, (worker, request) => {
		if (request.type === 'generate')
			worker.send(request, 'token', { text: 'answer' });
		worker.send(request, 'done');
	});
	const client = new InferenceClient({ kind: 'text-generation', device: 'webgpu' });
	t.after(() => client.dispose());
	const tokens = [];
	await client.run('generate', { messages: ['first'] }, text => tokens.push(text));
	await client.run('generate', { messages: ['second'] }, text => tokens.push(text));
	assert.equal(all.length, 1);
	assert.equal(all[0].requests.filter(request => request.type === 'init').length, 1);
	assert.equal(all[0].requests.filter(request => request.type === 'generate').length, 2);
