...
export class InferenceClient {
	...
	private request(type: string, payload: Record<string, unknown>, onToken?: (text: string) => void): Promise<any> {
		...
		return new Promise((resolve, reject) => {
			...
			const resetTimeout = () => {
				if (type !== 'generate' && type !== 'init')
					return;
				clearTimeout(timeout);
				const duration = type === 'init' ? this.options.initializationTimeoutMs ?? 300_000 : this.options.generationTimeoutMs ?? 120_000;
				timeout = setTimeout(() => fail(Object.assign(new Error(`本地模型${type === 'init' ? '加载或预热' : '推理'}长时间无响应，已停止。请检查网络、缩短问题，或切换 CPU/GPU 后重试。`), { name: 'InferenceTimeoutError' })), duration);
			};
		...
		const task = async () => {
			...
			try {
				...
				const retry = normalizeDevice(this.options.device) === 'auto' && !this.cpuFallback
					...
					&& !['AbortError', 'ModelDownloadError', 'ModelCacheError', 'ModelMemoryError', 'InferenceTimeoutError'].includes((error as Error).name);
				this.destroy();
				if (!retry)
					throw error;
